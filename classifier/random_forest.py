...
import pandas as pd
from sklearn.ensemble import RandomForestClassifier # Modèle utilisé
...
def classifier_training_rf(json_file):
    ...
    print(f"Dimensions de X: {X.shape}") # Doit être (nb_samples, 771)
    ...
    Y_proba_all = rf_model.predict_proba(X_test)
    ...
    print(f"Sensibilité (Recall): {score:.4f}")
    ...
    plt.title("Courbe ROC (Random Forest - Ensemble de Test)")
    ...
    plt.title(f"Courbe ROC Agrégée par {'id_cas'} (Random Forest)")
    plt.grid(True)
    plt.show()
...
def all_probabilities(json_file, rf_model, X, pipeline):
    output_dir = "RF_probabilities"
    ...
    for i, original_entry in enumerate(data):
        ...
        if pipeline == "camemBERT":
            ...
            output_filename_csv = "RF_probabilites_BERT.csv"
...
if __name__ == "__main__":
    ...
    pipeline = "camemBERT"
    X, Y, svm_model = classifier_training_rf(data_file)
